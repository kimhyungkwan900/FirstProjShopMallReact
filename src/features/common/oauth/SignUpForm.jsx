...
const SignUpForm = ({onSignUp}) => {
    ...
    const isFormValid = () => {
      ...
      const idValid = /^[a-zA-Z0-9_]{6,20}$/.test(user_id);
      const pwValid = user_password.length >= 8 && user_password.length <= 20;
    ...
    return(
      <div className="max-w-sm mx-auto mt-10 p-6 bg-white rounded-xl shadow-md">
        ...
        <form onSubmit={onSubmit} noValidate>
          ...
          <input
            type="password" name="user_password" placeholder="비밀번호 (6~20자)" value={form.user_password} onChange={onChange} minLength={8} maxLength={20} title="비밀번호는 6~20자 사이여야 합니다."
            className="w-full px-4 py-2 mb-3 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400" required/>
          <input
